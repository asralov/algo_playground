 ...
 * @file PathFinder.js
 * It has algorithms for matrix searching. DFS, BFS and A* are implemented and used
 * to search between start and goal cells in the 2d maze we are given. A maze consist
 * of 0s and 1s, where 1s represent walls, and 0s represent walkable areas.
 */

// a helper function to have a (row, col) style when working
// with columsn and rows. Useful to work with that format
function key(node) {
...
function Astar(maze, start, goal) {
  const openSet = []; // priority queue (we will sort by fScore)
